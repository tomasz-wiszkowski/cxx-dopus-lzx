...
class CustomColumn {
 public:
  using ValueFunc = std::function<size_t(const T&, wchar_t*, size_t)>;
...
class CustomColumnManager {
 ...
 public:
  ...
  void add_custom_column(const wchar_t* name, const wchar_t* key, DWORD flags, typename CustomColumn<T>::ValueFunc value_func) {
    auto new_column = std::make_unique<CustomColumn<T>>(
        static_cast<int>(columns_.size()), name, key, flags, std::move(value_func));
    if (!columns_.empty()) {
